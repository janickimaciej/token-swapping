...
{
	std::vector<std::pair<int, int>> IndependentSort::getSolution(Instance instance)
	{
	...
	void IndependentSort::checkBestMove(const Instance& instance, const std::pair<int, int>& move,
		int changeInDistance, std::pair<int, int>& bestMove, int& maxChangeInReverseTriples)
	{
		...
		{
			int changeInReverseTriples = IndependentReverseTriples::changeInCount(instance, move);
